...
namespace WhatIfBudget.API.Test
{
    ...
    public class MonthlyExpenseControllerTest
    {
        ...
        {
            //mock expense logic
            var mockMonthlyExpenseLogic = new Mock<IMonthlyExpenseLogic>();
            mockMonthlyExpenseLogic.Setup(x => x.GetBudgetMonthlyExpense(It.IsAny<int>())).Returns(5000);

            //Setup the http context (for auth)
            var monthlyExpenseController = new MonthlyExpenseController(mockMonthlyExpenseLogic.Object)
            {
