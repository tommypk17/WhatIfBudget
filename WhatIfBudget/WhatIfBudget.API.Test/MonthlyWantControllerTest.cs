...
{
    [TestClass]
    public class MonthlyWantControllerTest
    {
        [TestMethod]
        public void Get_BudgetWantExpense()
        {
            //mock expense logic
            var mockMonthlyWantLogic = new Mock<IMonthlyWantLogic>();
            mockMonthlyWantLogic.Setup(x => x.GetBudgetMonthlyWant(It.IsAny<int>())).Returns(5000);

            //Setup the http context (for auth)
            var monthlyWantController = new MonthlyWantController(mockMonthlyWantLogic.Object)
            {
